...
dpg.create_context()
dpg.create_viewport(title="Musicker",large_icon="favicon.ico",small_icon="favicon.ico")
pygame.mixer.init()

global state
state=None

_DEFAULT_MUSIC_VOLUME = 0.2
...
def load_database():
	...
	for filename in songs:
		...
		dpg.add_spacer(height=5, parent="list")
...
def search(sender, app_data, user_data):
	...
	for index, song in enumerate(songs):
		if app_data in song.lower():
			dpg.add_button(label=f"{ntpath.basename(song)}", callback=play,width=-1.5, height=40, user_data=song, parent="list")
			dpg.add_spacer(height=2,parent="list")
...
with dpg.theme(tag="base"):
	with dpg.theme_component():
		dpg.add_theme_color(dpg.mvThemeCol_Button, (250, 0, 171))
		dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (0, 248, 128, 95))
		dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (0, 248, 128))
...
with dpg.theme(tag="slider_thin"):
	with dpg.theme_component():
		dpg.add_theme_color(dpg.mvThemeCol_FrameBgActive, (250, 0, 171))
		dpg.add_theme_color(dpg.mvThemeCol_FrameBgHovered, (0, 248, 128))
		dpg.add_theme_color(dpg.mvThemeCol_SliderGrabActive, (255, 255, 255))
		dpg.add_theme_color(dpg.mvThemeCol_SliderGrab, (255, 255, 255))
		dpg.add_theme_color(dpg.mvThemeCol_FrameBg, (250, 0, 171))
...
with dpg.font_registry():
	monobold = dpg.add_font("fonts/MonoLisa-Bold.ttf", 22)
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=500,tag="sidebar"):
			dpg.add_text("Musicker",color=(250, 0, 171))
			dpg.add_text("Build by Nishant4coding")
			dpg.add_spacer(height=20)
			dpg.add_separator()
			# dpg.add_image("logo_image", value="music.png", width=2, height=10)
			dpg.add_spacer(height=2)
			dpg.add_spacer(height=5)
			dpg.add_separator()
			dpg.add_spacer(height=20)
			dpg.add_button(label="Add a Song",width=-1,height=50,callback=add_files)
			dpg.add_button(label="Add Folder",width=-1,height=50,callback=add_folder)
			dpg.add_button(label="Remove All Songs",width=-1,height=50,callback=removeall)
			dpg.add_spacer(height=20)
			dpg.add_separator()
			...
			dpg.add_button(label="Support",width=-1,height=60,callback=lambda:webbrowser.open(url="https://github.com/Nishant4coding"))

		with dpg.child_window(autosize_x=True,border=False):
			with dpg.child_window(autosize_x=True,height=80,no_scrollbar=True):
				with dpg.group(horizontal=True):
					dpg.add_button(label="Play",width=65,height=40,tag="play",callback=play_pause)
					dpg.add_button(label="Stop",callback=stop,width=65,height=40)
					dpg.add_slider_float(tag="volume", width=120,height=40,pos=(160,19),format="%.0f%.0%",default_value=_DEFAULT_MUSIC_VOLUME * 100,callback=update_volume)
			...
			with dpg.child_window(autosize_x=True,delay_search=True):
				with dpg.group(horizontal=True,tag="query"):
					dpg.add_input_text(hint="Search for a song",width=-1,height=40,callback=search)
				dpg.add_spacer(height=5)
